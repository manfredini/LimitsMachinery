...
function sedAndCat() {
    if [ "${9}" == "0"  ]; then
	sed -e "s/OverallSys Name=\"XS_bbA\".*$/OverallSys Name=\"XS_bbA\" High=\"${2}\" Low=\"${3}\" \/>/" -e "s/OverallSys Name=\"XS_ggA\".*$/OverallSys Name=\"XS_ggA\" High=\"${5}\" Low=\"${6}\" \/>/" -e "s/NormFactor Name=\"mbbA${TAUTAU_TAG}${8}\".*$/NormFactor Name=\"mbbA${TAUTAU_TAG}${8}\"  Val=\"${4}\" Low=\"${4}\" High=\"${4}\" \/>/" -e "s/NormFactor Name=\"mggA${TAUTAU_TAG}${8}\".*$/NormFactor Name=\"mggA${TAUTAU_TAG}${8}\"  Val=\"${7}\" Low=\"${7}\" High=\"${7}\" \/>/" ${1}
    else
	sed -e "/OverallSys Name=\"XS_bbA/d" -e "/OverallSys Name=\"XS_ggA/d" -e "s/NormFactor Name=\"mbbA${TAUTAU_TAG}${8}\".*$/NormFactor Name=\"mbbA${TAUTAU_TAG}${8}\"  Val=\"${4}\" Low=\"${4}\" High=\"${4}\" \/>/" -e "s/NormFactor Name=\"mggA${TAUTAU_TAG}${8}\".*$/NormFactor Name=\"mggA${TAUTAU_TAG}${8}\"  Val=\"${7}\" Low=\"${7}\" High=\"${7}\" \/>/" ${1}
    fi
    #echo "s/NormFactor Name=\"mggAlh${8}\".*$/NormFactor Name=\"mggAlh${8}\"  Val=\"${7}\" Low=\"${7}\" High=\"${7}\" \/>/"
}
...
function makeCrossSectionFile() {
    ...
    echo '</Channel>'  >> ${5}
    echo "remove the cross section normalization"
    if [ "${3}" == "lephad" ]; then
	sed -i '/NormFactor Name="mbbAlh/d' ${5}
	sed -i '/NormFactor Name="mggAlh/d' ${5}
#	sed -i temp'/NormFactor Name="mbbAlh/d' ${5}
#	sed -i temp '/NormFactor Name="mggAlh/d' ${5}
    elif [ "${3}" == "leplep" ]; then
	sed -i '/NormFactor Name="mbbAll/d' ${5}
	sed -i '/NormFactor Name="mggAll/d' ${5}
#	sed -i temp '/NormFactor Name="mbbAll/d' ${5}
#	sed -i temp '/NormFactor Name="mggAll/d' ${5}
    fi
    echo "remove the cross section error"
    sed -i '/\"XS_ggA\"/d' ${5}
    sed -i '/\"XS_bbA\"/d' ${5}
#    sed -i temp '/\"XS_ggA\"/d' ${5}
#    sed -i temp '/\"XS_bbA\"/d' ${5}
    echo "change the names of the signal histos - i.e. not to be weighted by the cross section"
...
#    sed -i temp 's/ATLAS_Sig_${FSTATE_LABEL}_ggA/ATLAS_Sig_{FSTATE_LABEL}_XSec_ggA/g' ${5}
#    sed -i temp 's/ATLAS_Sig_${FSTATE_LABEL}_bbA/ATLAS_Sig_{FSTATE_LABEL}_XSec_bbA/g' ${5}
}
...
#    outFolder="mAtb_Xmls_${THEOPTION_}"
	...
	echo "                     INSIG=$INSIG"
	echo "                     OUTXSbb=$OUTXSbb"
	echo "                     OUTXSgg=$OUTXSgg"
	
	if [ "${THEOPTION_}" == "1"  ]; then
	    echo "making the Cross Section Files"
